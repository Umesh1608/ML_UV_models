...
\begin{figure*}[!ht]
    ...
    \node[box, fill=green!6, draw=green!40!black, minimum width=4.4cm] (fp) at (9.5, 1.8)
    ...
    \node[box, fill=red!6, draw=red!40!black, minimum width=4.4cm] (mg) at (9.5, 0)
        {Molecular Graph\\[-1pt]{\scriptsize Atom/bond features per molecule, separate encoders}};

    \draw[arr, draw=red!40!black] (fork) -- (graphnode);
    ...
    \node[box, fill=blue!6, draw=blue!40!black, minimum width=4.4cm] (seq) at (9.5, -1.8)
    ...
    % ---- Five models in three groups (equal 1.3 spacing) ----
    \node[box, fill=green!12, draw=green!40!black] (rf) at (0, 2.6)
        {\textbf{Random Forest}\\[-1pt]{\scriptsize 1000 trees, \texttt{max\_features}$= 0.3d$}};

    \node[box, fill=green!12, draw=green!40!black] (xgb) at (0, 1.3)
    ...
    \node[box, fill=blue!12, draw=blue!40!black] (bigru) at (0, -1.3)
        {\textbf{BiGRU}\\[-1pt]{\scriptsize 3$\times$256 BiGRU, 3.0M params}};

    \node[box, fill=blue!12, draw=blue!40!black] (cb) at (0, -2.6)
    ...
    \node[font=\scriptsize\bfseries, green!40!black, rotate=90, anchor=center,
          text width=, minimum height=0pt] at (-3.5, 1.95) {Fingerprint};
    ...
    \node[font=\scriptsize\bfseries, blue!40!black, rotate=90, anchor=center,
          text width=, minimum height=0pt] at (-3.5, -1.95) {Sequence};

    % ---- Dashed separators ----
    \draw[dashed, gray!30, thin] (-3.2, 0.65) -- (11.5, 0.65);
    \draw[dashed, gray!30, thin] (-3.2, -0.65) -- (11.5, -0.65);

    % ---- Output box ----
    \node[box, fill=orange!8, draw=orange!50!black, text width=2.0cm,
          minimum height=2.8cm] (out) at (6.8, 0)
        {\textbf{\large $\hat{\lambda}_{\max}$}\\[3pt]{\scriptsize UV absorption}\\{\scriptsize wavelength (nm)}};

    % ---- Arrows: five models to output ----
    \draw[arr, draw=green!40!black] (rf.east)       -- (3.6, 2.6)   |- ([yshift=10pt]out.west);
    \draw[arr, draw=green!40!black] (xgb.east)      -- (3.6, 1.3)   |- ([yshift=5pt]out.west);
    \draw[arr, draw=red!40!black]   (chemprop.east) -- (3.6, 0)     |- (out.west);
    \draw[arr, draw=blue!40!black]  (bigru.east)    -- (3.6, -1.3)  |- ([yshift=-5pt]out.west);
    \draw[arr, draw=blue!40!black]  (cb.east)       -- (3.6, -2.6)  |- ([yshift=-10pt]out.west);
    ...
    % ---- Stage labels ----
    \node[font=\footnotesize\bfseries, gray!60, text width=, minimum height=0pt] at (0, -3.5) {\textsc{Models}};
    \node[font=\footnotesize\bfseries, gray!60, text width=, minimum height=0pt] at (8.0, -3.2) {\textsc{Prediction}};
