...
To assess whether pretraining on large molecular corpora bridges the gap between sequence models and fingerprint-based approaches, we include ChemBERTa~\cite{chithrananda2020chemberta}, a RoBERTa~\cite{liu2019roberta} model pretrained on SMILES from the ZINC database~\cite{irwin2005zinc}.
...
26,369 TD-DFT computed absorption wavelengths with 3D molecular geometries~\cite{potapov2026nablacolors}, featuring a published UniProp 3D GNN baseline (a solvent-aware variant of UniMol+; MAE = 15.97~nm, RMSE = 27.2~nm).
...
ChemBERTa, despite being pretrained on SMILES from the ZINC database and having 44.1M parameters (15$\times$ more than tuned BiGRU), achieves RMSE = 50.39~nm, substantially worse than both the tuned BiGRU (34.71~nm) and RF (31.34~nm).
...
\multirow{3}{*}{\shortstack[l]{nablaColors\\(scaffold)}}
  & UniProp$^\dagger$ (3D GNN)& 27.2  & 15.97  & 0.929   \\
...
On the nablaColors dataset~\cite{potapov2026nablacolors}, which uses a scaffold-based split designed to test generalization across structural classes, both 1D models perform comparably (RF RMSE = 56.38~nm, BiGRU = 56.57~nm) but lag far behind the published UniProp 3D GNN baseline (RMSE = 27.2~nm, MAE = 15.97~nm).
This $\sim$2.1$\times$ RMSE gap arises because the scaffold split pushes models to predict chromophores with no structurally similar training examples, and 3D conformational information (captured by UniProp but not by SMILES or fingerprints) becomes critical for diverse chromophore geometries.
