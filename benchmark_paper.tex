...
\begin{figure*}[t]
    ...
    % ---- Concat node ----
    \node[draw, circle, fill=gray!12, inner sep=3pt, font=\small\bfseries] (cat) at (3.8, 0) {$\oplus$};
    \node[font=\scriptsize, gray!50, below=3pt of cat] {concatenate};

    % Input arrows: horizontal then vertical into concat
    \draw[arr] (mol.east) -- (2.6, 1.2) -- (2.6, 0) -- (cat.west);
    \draw[arr] (solv.east) -- (2.6, -1.2) -- (2.6, 0);

    % ---- Fork point ----
    \coordinate (fork) at (5.0, 0);
    \draw[thick, gray!50] (cat.east) -- (fork);

    % ---- Representation boxes ----
    \node[box, fill=green!6, draw=green!40!black, minimum width=4.4cm] (fp) at (9.0, 1.5)
        {Morgan Fingerprint\\[-1pt]{\scriptsize $\vect{x} \in \{0,1\}^{4096}$, radius-2 atom neighborhoods}};

    \node[box, fill=blue!6, draw=blue!40!black, minimum width=4.4cm] (seq) at (9.0, -1.5)
        {Token Sequence\\[-1pt]{\scriptsize\texttt{! C C ( / C = C / \ldots ! C C O E}, 57-token vocab.}};

    % Fork arrows: vertical then horizontal, no overlap
    \draw[arr, draw=green!40!black] (fork) -- (5.0, 1.5) -- (fp.west);
    \draw[arr, draw=blue!40!black] (fork) -- (5.0, -1.5) -- (seq.west);

    % ---- Stage labels ----
    \node[font=\footnotesize\bfseries, gray!60] at (0, -2.2) {\textsc{Input}};
    \node[font=\footnotesize\bfseries, gray!60] at (9.0, -2.5) {\textsc{Representation}};

    \end{tikzpicture}
    \caption{Input processing and molecular representation. Solute and solvent SMILES are concatenated, then encoded as either hashed Morgan fingerprints (green, top) or character-level token sequences (blue, bottom).}
    ...
    % ---- Four models in a vertical stack ----
    % Green models (from fingerprint)
    \node[box, fill=green!12, draw=green!40!black, minimum width=4.2cm] (rf) at (0, 2.2)
        {\textbf{Random Forest}\\[-1pt]{\scriptsize 1000 trees, \texttt{max\_features}$\,= 0.3d$, 15 min CPU}};

    \node[box, fill=green!12, draw=green!40!black, minimum width=4.2cm] (xgb) at (0, 0.7)
        {\textbf{XGBoost}\\[-1pt]{\scriptsize 500 rounds, depth 6, $\eta = 0.1$, 20 min CPU}};

    % Blue models (from token sequence)
    \node[box, fill=blue!12, draw=blue!40!black, minimum width=4.2cm] (bigru) at (0, -0.8)
        {\textbf{BiGRU}\\[-1pt]{\scriptsize 2$\times$128 bidirectional GRU, 470K params, 25h GPU}};

    \node[box, fill=blue!12, draw=blue!40!black, minimum width=4.2cm] (cb) at (0, -2.3)
        {\textbf{ChemBERTa}\\[-1pt]{\scriptsize 6-layer Transformer, 44.1M params, 15h GPU}};

    % ---- Group labels on the left ----
    \node[font=\scriptsize\bfseries, green!40!black, rotate=90, anchor=south] at (-3.3, 1.45) {Fingerprint-based};
    \node[font=\scriptsize\bfseries, blue!40!black, rotate=90, anchor=south] at (-3.3, -1.55) {Sequence-based};

    % ---- Dashed separator between green and blue families ----
    \draw[dashed, gray!30, thin] (-3.0, -0.05) -- (12.5, -0.05);

    % ---- Output box ----
    \node[box, fill=orange!8, draw=orange!50!black, minimum width=2.8cm, minimum height=2.8cm] (out) at (7.0, 0)
        {\textbf{\large $\hat{\lambda}_{\max}$}\\[4pt]{\scriptsize UV absorption}\\{\scriptsize wavelength (nm)}};

    % ---- Arrows: each model to output, distinct bend x-positions ----
    \draw[arr, draw=green!40!black] (rf.east)    -- (4.0, 2.2) -- (4.0, 0.9)  -- ([yshift=9pt]out.west);
    \draw[arr, draw=green!40!black] (xgb.east)   -- (3.6, 0.7) -- (3.6, 0.3)  -- ([yshift=3pt]out.west);
    \draw[arr, draw=blue!40!black]  (bigru.east)  -- (3.6, -0.8) -- (3.6, -0.3) -- ([yshift=-3pt]out.west);
    \draw[arr, draw=blue!40!black]  (cb.east)     -- (4.0, -2.3) -- (4.0, -0.9) -- ([yshift=-9pt]out.west);
    ...
    % ---- Stage labels ----
    \node[font=\footnotesize\bfseries, gray!60] at (0, -3.2) {\textsc{Models}};
    \node[font=\footnotesize\bfseries, gray!60] at (8.5, -3.2) {\textsc{Prediction}};
    ...
    \caption{Benchmark framework for UV absorption prediction. (a)~Solute and solvent SMILES are concatenated and encoded through two representation pathways. (b)~Four models spanning classical ML and deep learning predict $\lambda_{\max}$ from the same molecular input.}
