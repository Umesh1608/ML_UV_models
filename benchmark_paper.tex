...
\begin{itemize}
    ...
    \item Empirical evidence that UV absorption prediction is a local-feature-dependent task~\cite{chenMolecularLanguage2023}: the strong performance of local-substructure models (Morgan fingerprints, BiGRU) and the interpretable mapping of top features to known chromophore motifs support the theoretical prediction that local-feature architectures are well-suited for properties determined by local electronic structure.
\end{itemize}
...
Chen et al.~\cite{chenMolecularLanguage2023} systematically compared RNNs and Transformers for molecular language modeling, finding that RNNs better capture local structural features while Transformers excel at global molecular properties and longer sequences.
Their conclusion---that the optimal architecture depends on the data characteristics---is particularly relevant to UV absorption prediction, where $\lambda_{\max}$ is fundamentally determined by local chromophore features: conjugation length, aromatic character, and heteroatom substituents, as formalized in the classical Woodward--Fieser rules.
This suggests that models encoding local structural information (fingerprints, RNNs) may have an inherent advantage over global-attention architectures for this property.
...
Our study extends this perspective specifically to UV absorption prediction, providing controlled comparisons across four diverse datasets with published baselines and complementary evaluation protocols.
...
Fine-tuning uses AdamW~\cite{loshchilov2019decoupled} with learning rate $5 \times 10^{-5}$, weight decay 0.01, linear warmup (200 steps) followed by linear decay, Huber loss ($\beta = 10$), batch size 32, and early stopping on validation MAE (patience 10 epochs, maximum 50 epochs).
...
\begin{figure}[htbp]
    ...
    \node[draw, rounded corners=2pt, fill=green!5, font=\scriptsize, align=center] (bigrures) at (0, -0.2) {RMSE = \textbf{36.45} nm\\(best on novel cpds)};
...
This 61\% gap relative to RF demonstrates that pretrained Transformer representations, which capture global molecular context through full self-attention, are poorly matched to the locality of UV absorption.
Notably, however, ChemBERTa achieves the lowest wetlab MAE on novel compounds (26.3~nm vs.\ 28.6~nm for BiGRU; Section~\ref{sec:wetlab}), suggesting that pretrained representations may offer complementary generalization advantages despite poor in-distribution performance.
...
The parity plot reveals strong overall agreement with a slight tendency to overpredict (mean signed error $> 0$), which is visible as a positive skew in the error distribution (Figure~\ref{fig:error_analysis}b).
...
This data-dependent model selection finding extends the conclusions of Chen et al.~\cite{chenMolecularLanguage2023}, who observed that RNNs outperform Transformers on local-feature-dependent molecular generation tasks.
Our regression benchmark demonstrates that this architectural trade-off persists in property prediction settings: since UV absorption depends on local chromophore features, models that capture local patterns (RF with fingerprints, BiGRU processing SMILES subsequences) achieve the best cross-validated performance.
...
More broadly, our results corroborate the growing evidence that model architecture choice in molecular ML should be guided by the locality of the target property~\cite{chenMolecularLanguage2023, jiang2023trangru}: properties determined by local substructures (UV absorption, functional group-dependent activity) favor fingerprint and RNN-based approaches, while properties depending on global molecular topology may benefit from Transformer or graph-based architectures.
...
Table~\ref{tab:perfold_rf} and Table~\ref{tab:perfold_bigru} present per-fold results for the RF and BiGRU models with solvent encoding on the primary Joung+Beard dataset (stratified 5-fold CV, 64/16/20 split).
...
\midrule
0 & 32.02 & 15.31 & 0.910 & 0.955 \\
1 & 30.74 & 14.86 & 0.918 & 0.958 \\
2 & 28.09 & 14.56 & 0.931 & 0.965 \\
3 & 32.58 & 15.68 & 0.907 & 0.953 \\
4 & 33.25 & 15.38 & 0.906 & 0.952 \\
\midrule
...
\textbf{Fold} & \textbf{RMSE (nm)} & \textbf{MAE (nm)} & $\boldsymbol{R^2}$ & \textbf{Pearson $r$} \\
\midrule
0 & 35.23 & 20.64 & 0.889 & 0.943 \\
1 & 37.33 & 21.30 & 0.879 & 0.937 \\
2 & 34.96 & 19.79 & 0.893 & 0.945 \\
3 & 37.62 & 21.13 & 0.876 & 0.936 \\
4 & 37.12 & 20.63 & 0.882 & 0.939 \\
\midrule
...
\midrule
RF vs.\ BiGRU (main)      & $-7.08$ & 0.002 & Yes \\
...
Tuning provides only an additional 2.6\% improvement, confirming that the performance gap reflects fundamental differences between fingerprint-based and sequence-based approaches rather than optimization effort.
