...
Chemprop, ChemBERTa, and BiGRU all outperform RF on this experimental validation set, with Chemprop achieving the lowest MAE (26.2~nm), followed closely by ChemBERTa (26.3~nm [95\% bootstrap CI: 20.1--33.0]), BiGRU (28.6~nm [22.7--35.0]), and RF (38.5~nm [31.6--45.9]).
...
This \textit{reverses} the ranking observed on the cross-validated primary dataset, where ChemBERTa was worst (RMSE = 50.39~nm) and Chemprop was tied with RF.
The reversal reflects a key difference: RF relies on exact substructure matching through Morgan fingerprint bits, excelling at interpolation within the training distribution but struggling when novel functional groups or conjugation patterns are underrepresented.
