...
\begin{figure*}[!ht]
    ...
    % ---- Input arrows: identical mirror-symmetric paths ----
    % Both: horizontal to x=2.5, then vertical to y=0 (mirrored)
    \draw[arr] (mol.east) -- (2.5, 1.0) -- (2.5, 0);
    \draw[arr] (solv.east) -- (2.5, -1.0) -- (2.5, 0);

    % ---- Merge point and line to fork ----
    \fill[gray!40] (2.5, 0) circle (3pt);
    \coordinate (fork) at (4.2, 0);
    \draw[thick, gray!50] (2.5, 0) -- (fork);
    ...
    \node[box, fill=green!12, draw=green!40!black] (rf) at (0, 1.7)
        {\textbf{Random Forest}\\[-1pt]{\scriptsize 1000 trees, \texttt{max\_features}$= 0.3d$}};

    \node[box, fill=green!12, draw=green!40!black] (xgb) at (0, 0.55)
        {\textbf{XGBoost}\\[-1pt]{\scriptsize 500 rounds, depth 6, $\eta = 0.1$}};

    \node[box, fill=blue!12, draw=blue!40!black] (bigru) at (0, -0.55)
        {\textbf{BiGRU}\\[-1pt]{\scriptsize 2$\times$128 BiGRU, 470K params}};

    \node[box, fill=blue!12, draw=blue!40!black] (cb) at (0, -1.7)
        {\textbf{ChemBERTa}\\[-1pt]{\scriptsize 6-layer Transformer, 44.1M params}};

    % ---- Group labels (rotated) ----
    \node[font=\scriptsize\bfseries, green!40!black, rotate=90, anchor=south,
          text width=, minimum height=0pt] at (-3.5, 1.12) {Fingerprint-based};
    \node[font=\scriptsize\bfseries, blue!40!black, rotate=90, anchor=south,
          text width=, minimum height=0pt] at (-3.5, -1.12) {Sequence-based};
    ...
    \node[box, fill=orange!8, draw=orange!50!black, text width=2.0cm,
          minimum height=2.2cm] (out) at (6.8, 0)
    ...
    \draw[arr, draw=green!40!black] (rf.east)     -- (3.6, 1.7)   |- ([yshift=7pt]out.west);
    \draw[arr, draw=blue!40!black]  (cb.east)     -- (3.6, -1.7)  |- ([yshift=-7pt]out.west);
    % Inner pair (XGBoost / BiGRU): yshift ±2pt
    \draw[arr, draw=green!40!black] (xgb.east)    -- (3.6, 0.55)  |- ([yshift=2pt]out.west);
    \draw[arr, draw=blue!40!black]  (bigru.east)  -- (3.6, -0.55) |- ([yshift=-2pt]out.west);
    ...
    % ---- Stage labels ----
    \node[font=\footnotesize\bfseries, gray!60, text width=, minimum height=0pt] at (0, -2.6) {\textsc{Models}};
    \node[font=\footnotesize\bfseries, gray!60, text width=, minimum height=0pt] at (8.0, -2.6) {\textsc{Prediction}};
