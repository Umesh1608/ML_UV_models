...
This locality distinction is architectural: fingerprints and D-MPNN have explicit neighborhood cutoffs (radius 2 and 3 hops, respectively), BiGRU has an implicit recency bias where nearby SMILES tokens dominate the hidden state, and Transformers attend globally with no distance penalty.
\end{abstract}
...
\begin{itemize}
    ...
    \item \textbf{Locality-of-property hypothesis:} architectures with local inductive bias systematically outperform global-attention Transformers on UV absorption, which is determined by local electronic structure~\cite{chenMolecularLanguage2023}. Fingerprints and D-MPNN have explicit neighborhood cutoffs (radius 2 and 3 hops), while BiGRU has an implicit recency bias where nearby SMILES tokens dominate the hidden state; Transformers attend globally with no distance penalty. RF feature importance and BiGRU gradient saliency independently highlight the same chromophore motifs, confirming that both architectures learn meaningful photochemistry.
