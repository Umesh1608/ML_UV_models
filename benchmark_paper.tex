...
ChemBERTa (pretrained)& XX.XX $\pm$ X.XX          & XX.XX $\pm$ X.XX          & X.XXX $\pm$ X.XXX          & X.XXX $\pm$ X.XXX \\
...
\begin{figure}[htbp]
    ...
    \node[draw, rounded corners=2pt, fill=red!5, font=\scriptsize, align=center] (bertres) at (4, -0.2) {RMSE = XX.XX nm\\(44.1M params)};
...
ChemBERTa, despite being pretrained on $\sim$77M SMILES strings from ZINC and having 44.1M parameters (94$\times$ more than BiGRU), achieves RMSE = XX.XX~nm---[PLACEHOLDER: discuss whether ChemBERTa beats/matches/loses to BiGRU and RF, and what this implies about pretrained representations vs.\ task-specific learning for UV prediction].
...
\multirow{5}{*}{Deep4Chem (80/10/10)}
  ...
  & Our ChemBERTa             & XX.XX  & XX.XX  & X.XXX \\
...
Across both external datasets, RF consistently outperforms BiGRU, reinforcing the finding from our primary dataset.
...
\midrule
BiGRU + Solvent   & \textbf{28.6} [22.7--35.0] & \textbf{33.9} [26.2--41.2] \\
ChemBERTa         & XX.X [XX.X--XX.X]          & XX.X [XX.X--XX.X] \\
RF + Morgan FP    & 38.5 [31.6--45.9]          & 43.7 [35.3--51.7] \\
\bottomrule
\end{tabular}
\end{table}

Notably, BiGRU outperforms RF on this experimental validation set (MAE = 28.6~nm [95\% bootstrap CI: 22.7--35.0] vs.\ 38.5~nm [31.6--45.9] for RF), \textit{reversing} the ranking observed on the cross-validated primary dataset.
The non-overlapping confidence intervals (10,000 bootstrap resamples) confirm that this difference is statistically significant.
...
Our regression benchmark demonstrates that this architectural trade-off persists in property prediction settings: since UV absorption depends on local chromophore features, models that capture local patterns (RF with fingerprints, BiGRU processing SMILES subsequences) are well-suited, while global self-attention mechanisms offer less advantage for a chromophore-localized property.
The complementarity extends further---beyond the local/global distinction---because RF's fixed-vocabulary substructure matching excels at interpolation while BiGRU's learned sequential patterns enable extrapolation to novel scaffolds.
...
\begin{enumerate}
    ...
    RF with Morgan fingerprints achieves the best cross-validated performance on the primary Joung+Beard dataset (RMSE = 31.34~nm vs.\ 36.45~nm for BiGRU) and on both external datasets, surpassing published deep learning baselines on Deep4Chem (RMSE = 22.70 vs.\ GCNN 26.6) and Jung 2024 (RMSE = 29.82 vs.\ GBFS 32.2).
