...
\begin{figure*}[!ht]
    ...
    % ---- Input boxes ----
    \node[box, fill=gray!6, draw=gray!50, minimum width=3.2cm] (mol) at (0, 1.0)
        {Solute SMILES\\[-1pt]{\scriptsize\texttt{CC(/C=C/c1cc(OC)c(O)c1)=O}}};
    \node[box, fill=gray!6, draw=gray!50, minimum width=3.2cm] (solv) at (0, -1.0)
        {Solvent SMILES\\[-1pt]{\scriptsize\texttt{CCO} (ethanol)}};

    % ---- Neutral merge point ----
    \fill[gray!40] (2.8, 0) circle (3pt);
    \draw[arr] (mol.east) -- (2.4, 1.0) -- (2.4, 0) -- (2.8, 0);
    \draw[arr] (solv.east) -- (2.4, -1.0) -- (2.4, 0);

    % ---- Fork line ----
    \coordinate (fork) at (4.2, 0);
    \draw[thick, gray!50] (2.8, 0) -- (fork);
    ...
    \node[box, fill=green!12, draw=green!40!black, minimum width=5.0cm, minimum height=0.9cm] (rf) at (0, 1.7)
        {\textbf{Random Forest}\\[-1pt]{\scriptsize 1000 trees, \texttt{max\_features}$\,= 0.3d$, 15 min CPU}};

    \node[box, fill=green!12, draw=green!40!black, minimum width=5.0cm, minimum height=0.9cm] (xgb) at (0, 0.55)
        {\textbf{XGBoost}\\[-1pt]{\scriptsize 500 rounds, depth 6, $\eta = 0.1$, 20 min CPU}};

    \node[box, fill=blue!12, draw=blue!40!black, minimum width=5.0cm, minimum height=0.9cm] (bigru) at (0, -0.55)
        {\textbf{BiGRU}\\[-1pt]{\scriptsize 2$\times$128 bidirectional GRU, 470K params, 25h GPU}};

    \node[box, fill=blue!12, draw=blue!40!black, minimum width=5.0cm, minimum height=0.9cm] (cb) at (0, -1.7)
        {\textbf{ChemBERTa}\\[-1pt]{\scriptsize 6-layer Transformer, 44.1M params, 15h GPU}};

    % ---- Group labels (rotated) ----
    \node[font=\scriptsize\bfseries, green!40!black, rotate=90, anchor=south] at (-3.5, 1.12) {Fingerprint-based};
    \node[font=\scriptsize\bfseries, blue!40!black, rotate=90, anchor=south] at (-3.5, -1.12) {Sequence-based};

    % ---- Dashed separator at y=0 ----
    \draw[dashed, gray!30, thin] (-3.2, 0) -- (11.5, 0);

    % ---- Output box ----
    \node[box, fill=orange!8, draw=orange!50!black, minimum width=2.4cm, minimum height=2.2cm] (out) at (6.5, 0)
        {\textbf{\large $\hat{\lambda}_{\max}$}\\[3pt]{\scriptsize UV absorption}\\{\scriptsize wavelength (nm)}};

    % ---- Arrows: mirror-symmetric ----
    % Outer pair (RF / ChemBERTa): bend at x=3.8
    \draw[arr, draw=green!40!black] (rf.east)  -- (3.8, 1.7)  |- ([yshift=7pt]out.west);
    \draw[arr, draw=blue!40!black]  (cb.east)  -- (3.8, -1.7) |- ([yshift=-7pt]out.west);
    % Inner pair (XGBoost / BiGRU): bend at x=3.3
    \draw[arr, draw=green!40!black] (xgb.east)   -- (3.3, 0.55)  |- ([yshift=2pt]out.west);
    \draw[arr, draw=blue!40!black]  (bigru.east)  -- (3.3, -0.55) |- ([yshift=-2pt]out.west);
    ...
    % ---- Stage labels ----
    \node[font=\footnotesize\bfseries, gray!60] at (0, -2.5) {\textsc{Models}};
    \node[font=\footnotesize\bfseries, gray!60] at (7.8, -2.5) {\textsc{Prediction}};
