...
\begin{figure*}[t]
    ...
    % ========== ROW POSITIONS ==========
    % Top row (fingerprint path): y = 1.8
    % Bottom row (sequence path):  y = -1.8
    % Column positions: input=0, concat=3, repr=6.5, model=10.5, output=14

    % ---- COLUMN 1: Input ----
    \node[box, fill=gray!6, draw=gray!50, minimum width=3cm] (mol) at (0, 0.7)
        {Solute SMILES\\[-1pt]{\scriptsize\texttt{CC(/C=C/c1cc(OC)c(O)c1)=O}}};
    \node[box, fill=gray!6, draw=gray!50, minimum width=3cm] (solv) at (0, -0.7)
        {Solvent SMILES\\[-1pt]{\scriptsize\texttt{CCO} (ethanol)}};
    \node[font=\footnotesize\bfseries, gray!60] at (0, -1.6) {\textsc{Input}};

    % ---- CONCAT ----
    \node[draw, circle, fill=gray!12, inner sep=3pt, font=\small\bfseries] (cat) at (3, 0) {$\oplus$};
    \node[font=\scriptsize, gray!50] at (3, -0.5) {concatenate};
    \draw[arr] (mol.east) -- (cat);
    \draw[arr] (solv.east) -- (cat);

    % ---- COLUMN 2: Representations ----
    \node[box, fill=green!6, draw=green!40!black, minimum width=3.4cm] (fp) at (6.5, 1.8)
        {Morgan Fingerprint\\[-1pt]{\scriptsize $\vect{x} \in \{0,1\}^{4096}$}\\[-1pt]{\scriptsize radius-2 atom neighborhoods}};

    \node[box, fill=blue!6, draw=blue!40!black, minimum width=3.4cm] (seq) at (6.5, -1.8)
        {Token Sequence\\[-1pt]{\scriptsize\texttt{! C C ( / \ldots ! C C O E}}\\[-1pt]{\scriptsize character-level encoding}};

    \draw[arr, draw=green!40!black] (cat) -- ++(0.6, 0) |- (fp.west);
    \draw[arr, draw=blue!40!black] (cat) -- ++(0.6, 0) |- (seq.west);

    \node[font=\footnotesize\bfseries, gray!60] at (6.5, -3.0) {\textsc{Representation}};

    % ---- COLUMN 3: Models ----
    % Top: tree ensembles
    \node[box, fill=green!12, draw=green!40!black, minimum width=2.4cm, font=\small\bfseries] (rf) at (10.5, 2.3)
        {Random Forest\\[-1pt]{\scriptsize\normalfont 1000 trees, 2048-bit FP}};
    \node[box, fill=green!12, draw=green!40!black, minimum width=2.4cm, font=\small\bfseries] (xgb) at (10.5, 1.0)
        {XGBoost\\[-1pt]{\scriptsize\normalfont 500 rounds, depth 6}};

    % Bottom: neural networks
    \node[box, fill=blue!12, draw=blue!40!black, minimum width=2.4cm, font=\small\bfseries] (bigru) at (10.5, -1.0)
        {BiGRU\\[-1pt]{\scriptsize\normalfont 2$\times$128 bidir., 470K params}};
    \node[box, fill=blue!12, draw=blue!40!black, minimum width=2.4cm, font=\small\bfseries] (cb) at (10.5, -2.3)
        {ChemBERTa\\[-1pt]{\scriptsize\normalfont 6-layer Transformer, 44.1M params}};

    % Arrows: representation -> models (straight horizontal)
    \draw[arr, draw=green!40!black] (fp.east) -- (rf.west);
    \draw[arr, draw=green!40!black] (fp.east) -- (xgb.west);
    \draw[arr, draw=blue!40!black] (seq.east) -- (bigru.west);
    \draw[arr, draw=blue!40!black] (seq.east) -- (cb.west);

    % Group labels
    \node[font=\scriptsize\bfseries, green!40!black] at (10.5, 3.1) {Tree ensembles};
    \node[font=\scriptsize\bfseries, blue!40!black] at (10.5, -3.1) {Neural networks};

    \node[font=\footnotesize\bfseries, gray!60] at (10.5, -3.6) {\textsc{Model}};

    % ---- COLUMN 4: Output ----
    \node[box, fill=orange!8, draw=orange!50!black, minimum width=2.2cm, minimum height=1.4cm] (out) at (14, 0)
        {\textbf{$\hat{\lambda}_{\max}$}\\[2pt]{\scriptsize UV absorption}\\{\scriptsize wavelength (nm)}};

    % Arrows: models -> output (with distinct vertical offsets to avoid overlap)
    \draw[arr, draw=green!40!black]  (rf.east)    -- ++(0.4, 0) |- ([yshift=4pt]out.west);
    \draw[arr, draw=green!40!black]  (xgb.east)   -- ++(0.6, 0) |- ([yshift=1.5pt]out.west);
    \draw[arr, draw=blue!40!black]   (bigru.east)  -- ++(0.6, 0) |- ([yshift=-1.5pt]out.west);
    \draw[arr, draw=blue!40!black]   (cb.east)     -- ++(0.4, 0) |- ([yshift=-4pt]out.west);

    \node[font=\footnotesize\bfseries, gray!60] at (14, -1.6) {\textsc{Prediction}};

    % ---- Dashed separator between the two pathways ----
    \draw[dashed, gray!30, thin] (2.2, 0) -- (15.3, 0);

    \end{tikzpicture}
    \caption{Benchmark framework for UV absorption prediction. Solute and solvent SMILES are concatenated and processed through two representation pathways: hashed Morgan fingerprints for tree-based ensembles (top, green), and character-level token sequences for neural network models (bottom, blue). All four models predict $\lambda_{\max}$ from the same molecular input, enabling controlled comparison of representation and architecture choices.}
