...
We present a systematic benchmark of machine learning approaches for $\lambda_{\max}$ prediction, comparing Random Forest (RF) with Morgan fingerprints, XGBoost, the Chemprop directed message passing neural network (D-MPNN), a Bidirectional GRU (BiGRU) sequence model, and ChemBERTa, a pretrained Transformer fine-tuned on SMILES strings.
...
RF and Chemprop achieve statistically indistinguishable cross-validated performance (RMSE = $31.34 \pm 1.82$ and $31.69 \pm 3.18$ nm, $p = 0.77$), both outperforming BiGRU (RMSE = $36.45 \pm 1.12$ nm) and matching or exceeding published deep learning baselines on external datasets, including GCNN on Deep4Chem (our RMSE = 22.70 vs.\ 26.6 nm) and GBFS on Jung et al.\ (our RMSE = 29.82 vs.\ 32.2 nm).
...
\textbf{Keywords:} UV absorption, molecular property prediction, deep learning, recurrent neural networks, random forest, Morgan fingerprints, graph neural network, Chemprop, ChemBERTa, solvent effects, benchmarking
...
\begin{figure*}[!ht]
    ...
    \end{tikzpicture}
    \caption{Models and prediction. Fingerprint representations feed tree ensembles (green); token sequences feed neural networks (blue). Chemprop (D-MPNN, not shown) operates directly on molecular graphs. All five models predict $\lambda_{\max}$.}
    \label{fig:framework_models}
    \end{subfigure}

    \caption{Benchmark framework for UV absorption prediction. (a)~Morgan fingerprints are computed independently per molecule and horizontally stacked (4096-bit), while token sequences are formed by string concatenation before tokenization. (b)~Four of five models are shown; Chemprop (D-MPNN) operates on molecular graphs with separate encoders for solute and solvent.}
...
\subsection{Graph Neural Network (Chemprop D-MPNN)}
\label{sec:chemprop}

To include a 2D graph-based representation, we evaluate Chemprop~\cite{yangAnalyzingLearnedMolecular2019}, the most widely adopted GNN for molecular property prediction.
Chemprop implements a Directed Message Passing Neural Network (D-MPNN) that operates on the molecular graph by passing messages along directed bonds rather than atoms, avoiding redundant information flow to the source atom at each step.
...
We use Chemprop v2 in a multicomponent configuration: separate D-MPNN encoders process the solute and solvent molecular graphs independently (hidden dimension $d_h = 300$ each), and their pooled representations are concatenated before a feed-forward regression head.
...
\begin{figure}[htbp]
    ...
    % --- Four model columns ---
    % RF column
    \node[localbox] (rf) at (-5.4, 1.8) {RF + Morgan FP};
    \node[annot, below=0pt of rf] (rfdesc) {Radius-2 substructures};
    \node[font=\scriptsize\bfseries, text=green!50!black] at (-5.4, 0.7) {LOCAL bits};

    % Chemprop column
    \node[localbox] (chemprop) at (-1.8, 1.8) {Chemprop (D-MPNN)};
    \node[annot, below=0pt of chemprop] (chempropdesc) {3-hop bond messages};
    \node[font=\scriptsize\bfseries, text=green!50!black] at (-1.8, 0.7) {LOCAL graph};

    % BiGRU column
    \node[localbox] (bigru) at (1.8, 1.8) {BiGRU (RNN)};
    \node[annot, below=0pt of bigru] (bigrudesc) {Sequential context};
    \node[font=\scriptsize\bfseries, text=green!50!black] at (1.8, 0.7) {LOCAL seq.};

    % ChemBERTa column
    \node[globalbox] (bert) at (5.4, 1.8) {ChemBERTa};
    \node[annot, below=0pt of bert] (bertdesc) {Full self-attention};
    \node[font=\scriptsize\bfseries, text=red!50!black] at (5.4, 0.7) {GLOBAL attn.};
    ...
    \node[draw, rounded corners=2pt, fill=green!5, font=\scriptsize, align=center] (rfres) at (-5.4, -0.2) {RMSE = \textbf{31.34} nm\\(best cross-val)};
    \node[draw, rounded corners=2pt, fill=green!5, font=\scriptsize, align=center] (chempropres) at (-1.8, -0.2) {RMSE = \textbf{31.69} nm\\(636K params)};
    \node[draw, rounded corners=2pt, fill=green!5, font=\scriptsize, align=center] (bigrures) at (1.8, -0.2) {RMSE = \textbf{34.71} nm\\(3.0M params)};
    \node[draw, rounded corners=2pt, fill=red!5, font=\scriptsize, align=center] (bertres) at (5.4, -0.2) {RMSE = \textbf{50.39} nm\\(44.1M params)};
...
This gap is consistent with the hypothesis that global-attention representations are less sample-efficient for local properties on moderately-sized datasets, though ChemBERTa's superior wetlab generalization (Section~\ref{sec:experimental}) suggests pretrained knowledge partially compensates.
ChemBERTa achieves the lowest wetlab MAE on novel compounds (26.3~nm vs.\ 28.6~nm for BiGRU; Section~\ref{sec:experimental}), suggesting that pretrained representations offer complementary generalization advantages despite poor in-distribution performance.
...
We have presented a systematic benchmark of ML approaches for UV absorption wavelength prediction, comparing Random Forest, XGBoost, Chemprop (D-MPNN), BiGRU, and ChemBERTa across four datasets totaling nearly 90,000 molecules.

Our key findings are:

\begin{enumerate}
    \item \textbf{RF and deep learning have complementary strengths.}
    RF with Morgan fingerprints and the Chemprop D-MPNN achieve statistically indistinguishable cross-validated performance on the primary Joung+Beard dataset (RMSE = 31.34 and 31.69~nm, $p = 0.77$), both outperforming the tuned BiGRU (34.71~nm) and ChemBERTa (50.39~nm). RF also surpasses published deep learning baselines on two external datasets: Deep4Chem (RMSE = 22.70 vs.\ GCNN 26.6~\cite{joung2021deep}) and Jung 2024 (RMSE = 29.82 vs.\ GBFS 32.2).
