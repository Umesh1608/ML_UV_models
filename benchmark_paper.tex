...
Potapov~\cite{potapov2026nablacolors} introduced the nablaColors dataset with 3D molecular geometries, where UniProp (a 3D GNN with 27.7M parameters) achieved MAE = 15.97~nm---demonstrating the significant advantage of 3D structural information for UV property prediction.
...
We also tested SELFIES~\cite{krenn2020selfies} (Self-Referencing Embedded Strings) as an alternative molecular representation on nablaColors, finding negligible difference from SMILES (MAE = 40.42 vs.\ 39.48~nm), suggesting that the representation bottleneck lies in the 1D nature of the encoding rather than the specific string format.
This is consistent with the observation of Lupopasini et al.~\cite{lupopasini2023excited} that excited-state properties are inherently geometry-dependent, requiring 3D molecular information for accurate prediction.
...
Our models use only 1D molecular representations, lacking 3D conformational information that the nablaColors comparison shows is important for diverse chromophores~\cite{potapov2026nablacolors}.
...
The BiGRU model uses character-level tokenization, which may miss chemically meaningful substructures compared to more sophisticated tokenization schemes such as those used in molecular Transformers~\cite{hondaSMILESTransformerPretrained2019}.
...
Incorporating 3D structural information through hybrid architectures~\cite{gilmer2017neural} or precomputed quantum-chemical descriptors~\cite{lupopasini2023excited} could address the limitations revealed by the nablaColors benchmark.
