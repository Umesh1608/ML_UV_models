...
RF with concatenated solute--solvent Morgan fingerprints achieves the best overall performance (RMSE = $31.34 \pm 1.82$ nm, $R^2 = 0.914$), outperforming BiGRU (RMSE = $36.45 \pm 1.12$ nm, $R^2 = 0.884$) and matching or exceeding published deep learning baselines on external datasets, including GCNN on Deep4Chem (our RMSE = 22.70 vs.\ 26.6 nm) and GBFS on Jung et al.\ (our RMSE = 29.82 vs.\ 32.2 nm).
We introduce a solvent concatenation strategy for sequence models, encoding solute and solvent as a single SMILES string with delimiters, yielding a consistent improvement across both BiGRU and RF.
Ablation studies show that incorporating solvent information reduces RMSE by 8\% for RF (31.34 vs.\ 34.13 nm) and X\% for BiGRU.
...
We further validate our models through ICH~S10 photosafety classification, matching published results (accuracy 0.879 vs.\ 0.89), and experimental validation on 16 UV-absorbing compounds measured in two solvents.
These results demonstrate that well-engineered classical ML can match or exceed deep learning for UV property prediction from 1D molecular representations, while highlighting scenarios where 3D structural information is essential.
\end{abstract}
...
\begin{itemize}
    ...
    \item Demonstration that RF with Morgan fingerprints consistently outperforms deep learning baselines, achieving state-of-the-art results on two external datasets.
    ...
    \item Practical validation through ICH~S10 photosafety classification and experimental UV measurements.
\end{itemize}
...
Incorporating solvent information reduces RF RMSE by 8\% (34.13 $\to$ 31.34~nm), confirming that the solvent concatenation strategy effectively captures solvatochromic effects for fingerprint-based models.
The BiGRU solvent ablation (v2, no data leakage) is currently running; the placeholder will be filled upon completion.
...
Interestingly, BiGRU outperforms RF on this experimental validation set (MAE = 28.6 vs.\ 38.5~nm overall), reversing the ranking observed on the cross-validated primary dataset.
This may reflect BiGRU's ability to generalize from learned sequential patterns to novel molecular structures, whereas RF is constrained by the specific substructural patterns present in its training fingerprints.
However, both models show substantially higher errors than their cross-validated performance, indicating that these 16 compounds are genuinely challenging out-of-distribution examples.
...
Octocrylene is particularly problematic for RF (error $\sim$89~nm), likely because its extended cyano-acrylate conjugation is underrepresented in the training data.
...
\begin{enumerate}
    \item \textbf{RF outperforms deep learning on 1D representations.}
    RF with Morgan fingerprints achieves the best performance on the primary Joung+Beard dataset (RMSE = 31.34~nm vs.\ 36.45~nm for BiGRU) and on all three external datasets.
    On Deep4Chem and Jung 2024, RF surpasses the published deep learning baselines (RMSE = 22.70 vs.\ GCNN 26.6; RMSE = 29.82 vs.\ GBFS 32.2), demonstrating that careful application of classical ML can yield state-of-the-art results.

    \item \textbf{Solvent encoding improves both model families.}
    The solvent concatenation strategy---encoding solute and solvent as a single representation---reduces RF RMSE by 8\% (34.13 $\to$ 31.34~nm).
    ...
    Furthermore, RF trains 100$\times$ faster than BiGRU (15 minutes CPU vs.\ 25 hours GPU) while achieving better accuracy, underscoring the value of evaluating classical models alongside deep learning to understand relative performance before committing to more complex architectures.
...
Transfer learning from larger UV absorption databases (e.g., the Reaxys dataset of $\sim$70K records) may improve BiGRU performance and narrow the gap with RF, leveraging deep learning's capacity for pretraining---an advantage that classical ML cannot replicate.
