...
We introduce a solvent concatenation strategy encoding solute and solvent as a single representation, yielding consistent improvement across all tested model families (8\% RMSE reduction for RF, 6\% for Chemprop, 7\% for BiGRU).
...
\begin{itemize}
    ...
    \item A solvent concatenation strategy that improves prediction accuracy across fingerprint, graph, and sequence-based methods.
...
The 24.6:1 solute-to-solvent importance ratio is consistent with the modest 6--8\% RMSE improvement observed in the solvent ablation (Section~\ref{sec:solvent_ablation}).
