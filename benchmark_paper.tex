...
In this work, we present a rigorous benchmark study comparing four ML approaches---Random Forest (RF), XGBoost, Bidirectional GRU (BiGRU), and ChemBERTa (a pretrained Transformer)---for $\lambda_{\max}$ prediction.
...
Importantly, the default RF configuration (scikit-learn defaults with $\sqrt{d}$ feature sampling) achieves RMSE = 32.18~nm; tuning improves this by only 2.6\%, indicating that RF's advantage over BiGRU is not an artifact of asymmetric hyperparameter optimization.
...
Notably, however, ChemBERTa achieves the lowest wetlab MAE on novel compounds (26.3~nm vs.\ 28.6~nm for BiGRU; Section~\ref{sec:experimental}), suggesting that pretrained representations may offer complementary generalization advantages despite poor in-distribution performance.
...
Notably, both deep learning models outperform RF on this experimental validation set, with ChemBERTa achieving the lowest MAE (26.3~nm [95\% bootstrap CI: 20.1--33.0]), followed by BiGRU (28.6~nm [22.7--35.0]) and RF (38.5~nm [31.6--45.9]).
...
The complementarity extends further---beyond the local/global distinction---because RF's fixed-vocabulary substructure matching excels at interpolation while learned representations (both BiGRU and ChemBERTa) enable extrapolation to novel scaffolds.
