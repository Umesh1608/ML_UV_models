...
\begin{table}[htbp]
\centering
\caption{Photosafety classification performance. Mamede et al.\ used POS = $\lambda_{\max} \in [290,700]$ nm AND MEC $\geq$ 1000. All models were trained and evaluated on the reconstructed Reaxys dataset with the same held-out test set.}
...
Per-fold cross-validation results for all four model families (Tables~S1--S5), statistical significance tests (Table~S6), RF hyperparameter sensitivity analysis, BiGRU and ChemBERTa learning curves (Figures~S1--S2), hyperparameter tuning asymmetry analysis (Table~S7), and BiGRU character-type saliency analysis (Figure~S3) (PDF).
\end{suppinfo}
