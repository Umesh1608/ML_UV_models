...
The Deep4Chem database contains 30,094 chromophore--solvent combinations~\cite{joung2021deep}; after deduplication and preprocessing, 17,683 records remain for benchmarking (reported GCNN baseline: RMSE = 26.6~nm).
...
On the nablaColors dataset~\cite{potapov2026nablacolors}, which uses a scaffold-based split designed to test generalization across structural classes, both 1D models perform comparably (RF RMSE = 56.38~nm, BiGRU = 56.57~nm) but lag far behind the published UniMol+ 3D GNN baseline (MAE = 15.97~nm).
This large gap ($\sim$2.5$\times$ higher RMSE than the published UniMol+ RMSE of 21.09~nm) arises because the scaffold split pushes models to predict chromophores with no structurally similar training examples, and 3D conformational information (captured by UniMol+ but not by SMILES or fingerprints) becomes critical for diverse chromophore geometries.
...
RF training completes in approximately 15 minutes on a single CPU, compared to 25 hours of GPU time for BiGRU (a 100$\times$ speedup) while achieving 10\% lower RMSE (Figure~\ref{fig:rf_practical}b).
...
We note that gradient saliency identifies correlations rather than causal mechanisms, and that attention weights in Transformer-based models are even less reliable for interpretability~\cite{jain2019attention} (see Supporting Information, Figure~S3, for character-type importance analysis).
...
We have presented a systematic benchmark of ML approaches for UV absorption wavelength prediction, comparing Random Forest, XGBoost, BiGRU, and ChemBERTa across four datasets totaling nearly 90,000 molecules.

Our key findings are:

\begin{enumerate}
    \item \textbf{RF and deep learning have complementary strengths.}
    RF with Morgan fingerprints achieves the best cross-validated performance on the primary Joung+Beard dataset (RMSE = 31.34~nm vs.\ 34.71~nm for the tuned BiGRU, 50.39~nm for ChemBERTa) and on both external datasets, surpassing published deep learning baselines on Deep4Chem (RMSE = 22.70 vs.\ GCNN 26.6~nm~\cite{joung2021deep}) and Jung 2024 (RMSE = 29.82 vs.\ GBFS 32.2~nm).
...
Additional external datasets: Jung et al.\ (GitHub)~\cite{jung2024machine}, nablaColors (Zenodo)~\cite{potapov2026nablacolors}.
