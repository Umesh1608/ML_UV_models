...
\keywords{UV absorption, molecular property prediction, deep learning, random forest, Morgan fingerprints, ChemBERTa, solvent effects, benchmarking}
...
% ===== Abstract =====
\begin{abstract}
We benchmark Random Forest (RF), XGBoost, a Bidirectional GRU (BiGRU) recurrent neural network optimized via Bayesian hyperparameter search, and ChemBERTa, a pretrained Transformer, for predicting UV absorption wavelengths ($\lambda_{\max}$) on 18,755 solute--solvent pairs with stratified 5-fold cross-validation and three external datasets totaling over 70,000 molecules; RF with concatenated solute--solvent Morgan fingerprints achieves the best cross-validated RMSE ($31.34 \pm 1.82$~nm, $R^2 = 0.914$) and surpasses published deep learning baselines on two external benchmarks, while hyperparameter optimization of BiGRU (3-layer, 256-unit architecture) reduces its RMSE by 8.6\% over the default configuration.
However, experimental validation on 16 novel UV-absorbing compounds reverses the ranking: both deep learning models outperform RF on out-of-distribution molecules (ChemBERTa MAE = 26.3, BiGRU = 28.6 vs.\ 38.5~nm), revealing that learned representations generalize beyond fixed substructural features.
A solvent concatenation strategy yields consistent 7--8\% RMSE reductions across model families, and interpretability analysis shows that RF feature importance and BiGRU gradient saliency independently highlight the same chromophore motifs, confirming that both architectures learn meaningful photochemistry.
These results demonstrate that models encoding local structural information (fingerprints, RNNs) outperform global-attention architectures on properties determined by local molecular features, while pretrained Transformers excel at out-of-distribution generalization---providing practical guidance for model selection in molecular property prediction beyond UV absorption.
\end{abstract}
...
Figure~\ref{fig:benchmark_framework} provides an overview of this dual-pathway framework.
...
Our evaluation methodology follows best practices for QSAR/QSPR studies~\cite{tropsha2006best, cherkasov2014qsar}: models are validated using both internal cross-validation and external test sets, multiple statistical metrics are reported, and applicability domain is assessed through out-of-distribution experimental validation (Section~\ref{sec:experimental}).
...
Approximately 20,000 UV absorption records with a reported GCNN baseline (RMSE = 26.6~nm)~\cite{joung2021deep}.
...
\textbf{Deep4Chem.}
Our RF model achieves RMSE = 22.70~nm, \textit{surpassing} the published GCNN baseline of 26.6~nm~\cite{joung2021deep} by 15\%.
...
We note that gradient saliency identifies correlations rather than causal mechanisms, and that attention weights in Transformer-based models are even less reliable for interpretability~\cite{jain2019attention} (see Supporting Information, Figure~S1, for character-type importance analysis).

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{results/bigru_saliency_atoms.pdf}
    \caption{BiGRU gradient saliency mapped to 2D molecular structures for six wetlab validation molecules. Color intensity indicates atom-level importance (red = high saliency, white = low). Top row (green borders): well-predicted molecules with focused saliency on chromophore regions. Bottom row (red borders): poorly predicted molecules with higher absolute errors. Error values are shown below each structure.}
...
The regression-based thresholding approach using BiGRU predictions proved ineffective (accuracy = 0.588, sensitivity = 0.22), as the regression model's prediction errors span both sides of the classification boundary, producing many false negatives.
...
\begin{enumerate}
    ...
    RF with Morgan fingerprints achieves the best cross-validated performance on the primary Joung+Beard dataset (RMSE = 31.34~nm vs.\ 36.45~nm for BiGRU, 50.39~nm for ChemBERTa) and on both external datasets, surpassing published deep learning baselines on Deep4Chem (RMSE = 22.70 vs.\ GCNN 26.6~\cite{joung2021deep}) and Jung 2024 (RMSE = 29.82 vs.\ GBFS 32.2).
...
More broadly, these findings offer practical guidance for molecular property prediction beyond UV absorption: when a target property is determined by local molecular features (conjugation, substituent effects, functional groups), models that encode local structural information---fingerprints with tree ensembles and RNNs processing SMILES subsequences---are expected to outperform global-attention architectures on in-distribution benchmarks, while pretrained Transformers retain advantages for out-of-distribution generalization to novel scaffolds.
Model selection should therefore be guided by (1)~the locality of the target property and (2)~the relationship between target compounds and available training data: local-feature models for interpolation within known chemical space, learned representations for extrapolation to novel architectures.
This framework should generalize to related molecular properties including emission wavelength, molar extinction coefficient, and other endpoints where the balance between local and global structural determinants varies.
...
External validation datasets: Deep4Chem (Figshare, DOI: 10.6084/m9.figshare.12045567)~\cite{joung2020experimental}, Jung et al.\ (GitHub)~\cite{jung2024machine}, nablaColors (Zenodo)~\cite{potapov2026nablacolors}.
...
Per-fold cross-validation results for all four model families (Tables~S1--S4), statistical significance tests (Table~S5), RF hyperparameter sensitivity analysis, BiGRU and ChemBERTa learning curves (Figures~S1--S2), hyperparameter tuning asymmetry analysis (Table~S6), and BiGRU character-type saliency analysis (Figure~S3) (PDF).
\end{suppinfo}
