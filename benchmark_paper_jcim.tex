...
\keywords{UV absorption, molecular property prediction, deep learning, random forest, Morgan fingerprints, graph neural network, Chemprop, ChemBERTa, solvent effects, benchmarking}
...
% ===== Abstract =====
\begin{abstract}
We benchmark Random Forest (RF), XGBoost, Chemprop (D-MPNN), a Bidirectional GRU (BiGRU) recurrent neural network optimized via Bayesian hyperparameter search, and ChemBERTa, a pretrained Transformer, for predicting UV absorption wavelengths ($\lambda_{\max}$) on 18,755 solute--solvent pairs with stratified 5-fold cross-validation and three external datasets totaling nearly 70,000 molecules; RF with concatenated solute--solvent Morgan fingerprints and the Chemprop graph neural network achieve statistically indistinguishable performance (RMSE = $31.34 \pm 1.82$ and $31.69 \pm 3.18$~nm, $p = 0.77$), both outperforming sequence-based models, while hyperparameter optimization of BiGRU (3-layer, 256-unit architecture) reduces its cross-validated RMSE by 4.8\% (to $34.71 \pm 1.40$~nm).
...
\begin{enumerate}
    ...
    RF with Morgan fingerprints and the Chemprop D-MPNN achieve statistically indistinguishable cross-validated performance on the primary Joung+Beard dataset (RMSE = 31.34 and 31.69~nm, $p = 0.77$), both outperforming the tuned BiGRU (34.71~nm) and ChemBERTa (50.39~nm). RF also surpasses published deep learning baselines on two external datasets: Deep4Chem (RMSE = 22.70 vs.\ GCNN 26.6~nm~\cite{joung2021deep}) and Jung 2024 (RMSE = 29.82 vs.\ GBFS 32.2~nm).
...
Per-fold cross-validation results for all five model families (Tables~S1--S6), statistical significance tests (Table~S7), RF hyperparameter sensitivity analysis, BiGRU and ChemBERTa learning curves (Figures~S1--S2), hyperparameter tuning asymmetry analysis (Table~S8), and BiGRU character-type saliency analysis (Figure~S3) (PDF).
\end{suppinfo}
