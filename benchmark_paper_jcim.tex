...
\begin{figure}[htbp]
    ...
    \caption{BiGRU gradient saliency mapped to 2D molecular structures for six wetlab validation molecules. Atom color intensity reflects gradient-based importance (red = high saliency). Across all six molecules, the model consistently highlights heteroatoms (O, N), carbonyl groups, and conjugated $\pi$-systems---the same chromophore motifs identified by RF feature importance (Section~\ref{sec:rf_interpret}). Top row: well-predicted molecules ($|\text{error}| < 22$~nm); bottom row: poorly predicted molecules ($|\text{error}| > 28$~nm). Saliency patterns are qualitatively similar regardless of prediction accuracy, indicating that the model attends to chemically relevant features even when its magnitude estimates are inaccurate.}
