...
Figure~\ref{fig:benchmark_framework} provides an overview of our dual-pathway benchmarking framework, illustrating how both molecular representations (fingerprints and token sequences) feed into the four model families evaluated in this study.
...
BiGRU (tuned, 3L/256u)& 34.71 $\pm$ 1.40          & 18.09 $\pm$ 0.59          & 0.890 $\pm$ 0.010          & 0.950 $\pm$ 0.003 \\
...
RF with Morgan fingerprints achieves the best performance across all metrics, with RMSE = 31.34~nm compared to 34.71~nm for the tuned BiGRU (36.45~nm default), a 10\% improvement.
...
\begin{figure}[htbp]
    ...
    \node[draw, rounded corners=2pt, fill=green!5, font=\scriptsize, align=center] (bigrures) at (0, -0.2) {RMSE = \textbf{34.71} nm\\(3.0M params, tuned)};
...
ChemBERTa, despite being pretrained on $\sim$77M SMILES strings from ZINC and having 44.1M parameters (15$\times$ more than tuned BiGRU), achieves RMSE = 50.39~nm, substantially worse than both the tuned BiGRU (34.71~nm) and RF (31.34~nm).
This 45\% gap relative to the tuned BiGRU is consistent with the hypothesis that Transformer representations are less sample-efficient for local properties on moderately-sized datasets, though ChemBERTa's superior wetlab generalization (Section~\ref{sec:experimental}) suggests pretrained knowledge partially compensates.
...
Notably, the tuned BiGRU (3L/256u) performs \textit{worse} on wetlab validation (MAE = 31.5~nm) than the default (28.6~nm), despite 4.8\% lower cross-validated RMSE---a classic bias--variance tradeoff where the larger architecture overfits to in-distribution patterns at the cost of out-of-distribution generalization.
...
\begin{enumerate}
    ...
    RF with Morgan fingerprints achieves the best cross-validated performance on the primary Joung+Beard dataset (RMSE = 31.34~nm vs.\ 34.71~nm for the tuned BiGRU, 50.39~nm for ChemBERTa) and on both external datasets, surpassing published deep learning baselines on Deep4Chem (RMSE = 22.70 vs.\ GCNN 26.6~\cite{joung2021deep}) and Jung 2024 (RMSE = 29.82 vs.\ GBFS 32.2).
    ...
    BiGRU, optimized via Bayesian HPO (3-layer, 256-unit architecture, 4.8\% RMSE improvement over the default on 5-fold CV), provides instance-level interpretability through gradient saliency analysis, with atom-level heatmaps highlighting the same conjugated regions identified by RF (Section~\ref{sec:interpretability}).
