...
% ===== Abstract =====
\begin{abstract}
We benchmark Random Forest (RF), XGBoost, a Bayesian-optimized Bidirectional GRU (BiGRU), and ChemBERTa for predicting UV absorption wavelengths ($\lambda_{\max}$) on 18,755 solute--solvent pairs with stratified 5-fold cross-validation and three external datasets totaling over 70,000 molecules; RF with concatenated solute--solvent Morgan fingerprints achieves the best cross-validated RMSE ($31.34 \pm 1.82$~nm, $R^2 = 0.914$) and surpasses published deep learning baselines on two external benchmarks, while Bayesian hyperparameter optimization of BiGRU (3-layer, 256-unit architecture) reduces its RMSE by 8.6\% over the default configuration.
