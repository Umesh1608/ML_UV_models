...
\begin{changes}
  \item \textbf{Results---new subsection.} Added ``Performance on Non-Local Subsets'' as a new subsection of Results and Discussion, immediately preceding ``Error Analysis.'' The subsection defines five non-local subsets via SMARTS, reports an in-text RMSE table (Table~\ref{tab:nonlocal_subsets}), and presents the two-trend discussion summarised above.
  \item \textbf{Supporting Information---new section.} Added ``SMARTS Rules and Metrics for Non-Local Subset Analysis'' (Section~\ref{si:nonlocal_smarts}) containing four tables: the full donor SMARTS list (Table~\ref{tab:si_donor_smarts}), the acceptor SMARTS list (Table~\ref{tab:si_acceptor_smarts}), the dye-motif SMARTS list (Table~\ref{tab:si_motif_smarts}), the full-dataset subset counts (Table~\ref{tab:si_subset_counts}), and a complete RMSE/MAE/$R^2$ breakdown across all subsets and all five models (Table~\ref{tab:si_nonlocal_full}).
