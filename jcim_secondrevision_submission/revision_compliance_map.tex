...
canonicalised).''
\where{Clean manuscript \textbf{p.~35} (Table 5 caption) $\cdot$ Marked \textbf{p.~41} $\cdot$
...
\textbf{pp.~33, 34, 37} $\cdot$ Response letter \textbf{p.~2} $\cdot$ Scripts
...
Marked \textbf{p.~40} against \textbf{p.~44} $\cdot$ Response letter \textbf{p.~3}}
...
\where{Clean manuscript \textbf{p.~57} (the final page) $\cdot$ Source
...
\where{Clean manuscript \textbf{p.~48} (Acknowledgement) $\cdot$ Marked \textbf{p.~55} $\cdot$
...
surfaced and listed in the Summary of Major Changes --- the ``variance is intrinsic to Chemprop'' claim,
corrected by the R1-12 HPO probe, and the 93.6\% Reaxys recovery claim, corrected in R1-15. The inference
about AI assistance is confirmed directly rather than deflected, and cross-referenced to the disclosure in
point~5.
\where{Response letter \textbf{p.~4} (Comment 1) $\cdot$ \textbf{p.~8}\,+ (Part B, every block verbatim)
$\cdot$ \textbf{p.~33} (Reviewer 2) $\cdot$ \textbf{p.~40} (Summary of Major Changes)}
...
\textbf{code development and debugging, data-analysis scripting, and editing of the manuscript and
response-to-reviewer text} --- explicitly covering the manuscript writing the reviewer inferred from the
commit history and CLAUDE.md, and covering the response document too. The authorship boundary is stated in
the same sentence: all scientific content, analysis, experimental work and interpretation are the authors'
own, and the authors take responsibility for the final manuscript.
\where{Clean manuscript \textbf{p.~48} (Acknowledgement) $\cdot$ Marked \textbf{p.~55} $\cdot$
...
Rebuilt from source: all fourteen citations resolve to numbered entries, the nine-entry References list is
restored, and the document runs 46 pages instead of 45. Old and new were diffed to confirm nothing else
changed. The cover letter was rebuilt at the same time so both now carry the 27 July submission date rather
than 15 July.
...
\midrule
\texttt{\ldots\_revised.pdf} & Manuscript & 57 & Verified \\
\texttt{\ldots\_revised\_marked.pdf} & Supp.\ Info.\ for Review Only & 63 & Verified \\
\texttt{\ldots\_response\_to\_reviewers.pdf} & Response to Reviewers & 46 & \textbf{Rebuilt} \\
